...
export async function POST(request: Request) {
  ...
  const { docId, chatId, messages, config, trigger, messageId, tempId } =
    requestBody;
      ...
      try {
        ...
        if (!activeChatId) {
          ...
          if (!activeDocId) {
            throw {
              context: "chat_create",
              original: new Error("Missing documentId"),
            };
          }
          ...
          if (chatError || !newChat) {
            ...
            throw {
              context: "chat_create",
              original: chatError || new Error("Failed to create chat"),
            };
          }
        ...
        // SCENARIO B: Active chat, derive documentId from it
          ...
          if (chatFetchError || !chatData) {
            ...
            throw {
              context: "chat_fetch",
              original:
                chatFetchError || new Error("Failed to fetch chat data"),
            };
          }
        ...
        const { data: chunks, error: matchingError } = await supabase.rpc(
          "match_documents",
          {
        ...
        const result = streamText({
          ...
          onFinish: async ({ text }) => {
            ...
            if (!isRegenerate) {
              ...
              if (userMsgError) {
                console.error(
                  "[Chat API] Failed to save user message:",
                  userMsgError,
                );
              }
        ...
        const context = (errObj?.context as string) || "unknown";
        const originalError = errObj?.original || err;
